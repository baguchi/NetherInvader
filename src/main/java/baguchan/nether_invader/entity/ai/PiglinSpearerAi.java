...
public class PiglinSpearerAi {
    ...
    private static ActivityData<PiglinSpearer> initCoreActivity() {
    ...
    private static ActivityData<PiglinSpearer> initIdleActivity() {
        return ActivityData.create(
                Activity.IDLE,
                10,
                ImmutableList.of(
                        SetEntityLookTarget.create(PiglinSpearerAi::isPlayerHoldingLovedItem, 14.0F),
                        StartAttacking.create((level, piglin) -> true, PiglinSpearerAi::findNearestValidAttackTarget),
                        BehaviorBuilder.triggerIf(PiglinSpearer::canHunt, StartHuntingHoglinUniversal.create()),
    ...
    private static ActivityData<PiglinSpearer> initFightActivity(PiglinSpearer body) {
        return ActivityData.create(
                Activity.FIGHT,
                10,
                ImmutableList.<BehaviorControl<? super PiglinSpearer>>of(
                        StopAttackingIfTargetInvalid.create((level, target) -> !isNearestValidAttackTarget(level, body, target)),
                        SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.0F),
                        new AttackWithAnimation<>(2, 20, 30, 0.8F),
                        RememberIfHoglinWasKilled.create()),
                MemoryModuleType.ATTACK_TARGET
        );
    }

    private static ActivityData<PiglinSpearer> initAdmireItemActivity() {
        return ActivityData.create(
                Activity.ADMIRE_ITEM,
                10,
                ImmutableList.of(
                        GoToWantedItem.create(PiglinSpearerAi::isNotHoldingLovedItemInOffHand, 1.0F, true, 9),
    ...
    private static RunOne<PiglinSpearer> createIdleMovementBehaviors() {
        return new RunOne<>(
                ImmutableList.of(
    ...
    public static void updateActivity(PiglinSpearer p_34899_) {
        Brain<PiglinSpearer> brain = p_34899_.getBrain();
    ...
    public static void pickUpItem(ServerLevel level, PiglinSpearer body, ItemEntity itemEntity) {
        stopWalking(body);
        ItemStack taken;
    ...
    private static void holdInOffhand(ServerLevel level, PiglinSpearer body, ItemStack itemStack) {
    ...
    public static InteractionResult mobInteract(ServerLevel level, PiglinSpearer body, Player player, InteractionHand hand) {
    ...
    private static boolean isNearestValidAttackTarget(ServerLevel serverLevel, PiglinSpearer piglin, LivingEntity target) {
        return findNearestValidAttackTarget(serverLevel, piglin).filter(p_34887_ -> p_34887_ == target).isPresent();
    }

    private static boolean isNearZombified(PiglinSpearer p_34999_) {
        Brain<PiglinSpearer> brain = p_34999_.getBrain();
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel level, PiglinSpearer body) {
        Brain<PiglinSpearer> brain = body.getBrain();
    ...
    public static void wasHurtBy(ServerLevel serverLevel, PiglinSpearer piglin, LivingEntity target) {
        if (!(target instanceof PiglinSpearer)) {

            Brain<PiglinSpearer> brain = piglin.getBrain();
    ...
    public static Optional<SoundEvent> getSoundForCurrentActivity(PiglinSpearer p_34948_) {
        return p_34948_.getBrain().getActiveNonCoreActivity().map(p_34908_ -> getSoundForActivity(p_34948_, p_34908_));
    }

    private static SoundEvent getSoundForActivity(PiglinSpearer p_34855_, Activity p_34856_) {
    ...
    private static boolean isNearAvoidTarget(PiglinSpearer p_35003_) {
        Brain<PiglinSpearer> brain = p_35003_.getBrain();
    ...
    protected static List<AbstractPiglin> getVisibleAdultPiglins(PiglinSpearer p_35005_) {
    ...
    private static void stopWalking(PiglinSpearer p_35007_) {
    ...
    public static Optional<LivingEntity> getAvoidTarget(PiglinSpearer p_34987_) {
    ...
    private static void broadcastRetreat(PiglinSpearer p_34930_, LivingEntity p_34931_) {
        getVisibleAdultPiglins(p_34930_)
                .stream()
                .filter(p_34985_ -> p_34985_ instanceof Piglin)
                .forEach(p_34819_ -> retreatFromNearestTarget((PiglinSpearer) p_34819_, p_34931_));
    }

    private static void retreatFromNearestTarget(PiglinSpearer p_34950_, LivingEntity p_34951_) {
        Brain<PiglinSpearer> brain = p_34950_.getBrain();
    ...
    public static boolean wantsToStopFleeing(PiglinSpearer p_35009_) {
        Brain<PiglinSpearer> brain = p_35009_.getBrain();
    ...
    private static boolean piglinsEqualOrOutnumberHoglins(PiglinSpearer p_35011_) {
        return !hoglinsOutnumberPiglins(p_35011_);
    }

    private static boolean hoglinsOutnumberPiglins(PiglinSpearer p_35013_) {
    ...
    private static void setAvoidTargetAndDontHuntForAWhile(PiglinSpearer p_34968_, LivingEntity p_34969_) {
    ...
    private static boolean isNearRepellent(PiglinSpearer p_35023_) {
    ...
    public static boolean isPlayerHoldingLovedItem(LivingEntity p_34884_) {
        return p_34884_.getType() == EntityType.PLAYER && p_34884_.isHolding(PiglinSpearerAi::isLovedItem);
    }
